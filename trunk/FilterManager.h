...
class FilterManager {
	...
	public:
		...
		// release resource
		//inline void endFind() {}
		bool doFilter(const KURL & mainURL, const String & key,
				const KURL & url, FilterType t);
	};
...
private:
	...
	DocumentFilterURLHashMap m_DocumentFilterURLs;
private:
	/*
...
public:
	 ...
	 * Besides of translating filters into regular expressions Adblock Plus also
	 ...
	 very recommendable.
	 */
	bool shouldFilter(const Document *doc,
			const ResourceResponseBase * response);
	bool haveFilted(const Document *, const String & url);
	void realseRecord(const Document *);
...
private:
	...
	void insertRuleToFilterRuleMap(FilterRuleMap * rules,
			const String & key, FilterRule * r);
	bool shouldFilterByShortcut(const KURL & mainURL, const KURL & url,
			FilterType t);
	bool shouldFilterByDomain(const KURL & mainURL, const KURL & url,
			FilterType t,bool & isFind);
